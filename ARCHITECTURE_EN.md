...
2. **Current-line highlight** — `LineNumberRulerView` adds a `currentLineNumber: Int` and a `var highlightCurrentLine: Bool = true`. A new observer on `NSTextView.didChangeSelectionNotification` recomputes the line by counting `\n`s before `selectedRange().location`; the ruler short-circuits the redraw when the line number hasn't changed. `drawNumber(_:atTextContainerY:textView:attrs:)` uses a semibold + `controlAccentColor` style for the matching number, falling back to the default tertiary gray otherwise.
3. **Plumbing** — `MarkdownEditor` exposes `var highlightCurrentLine: Bool = true` and pushes it to the ruler in `makeNSView` (initial) and `updateNSView` (when the binding flips). `ContentView` keeps `@SceneStorage("highlightCurrentLine")` and toggles it via `.toggleCurrentLineHighlight`, posted from a *View* menu item.
